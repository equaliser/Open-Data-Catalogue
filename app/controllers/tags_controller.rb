class TagsController < ApplicationController
  ...
  def initialize
   ...
   # GET /datasets.xml
   def index    
    @sorted_tags = Dataset.all(:select=>'tags.*, count(*) as count',:conditions => {:status => "Published"},:joins=>:tags, :group=>"tags.id, tags.name").sort { |x,y| x.name.downcase <=> y.name.downcase }
